...
class Average
{
    ...
    /**
     * Winsorized mean
     * The mean of a dataset after replacing the smallest and largest values
     * with the nearest values that are not being replaced.
     ...
     * @throws Exception\BadDataException if the input array of numbers is empty
     */
    public static function winsorizedMean(array $numbers, int $trim_percent): float
    {
        if (empty($numbers)) {
            throw new Exception\BadDataException('Cannot find the winsorized mean of an empty list of numbers');
        }
        if ($trim_percent < 0 || $trim_percent > 99) {
            throw new Exception\OutOfBoundsException('Trim percent must be between 0 and 99.');
        }
        ...
        sort($numbers);
        $low_val  = $numbers[$trim_count];
        $high_val = $numbers[$n - $trim_count - 1];

        for ($i = 0; $i < $trim_count; $i++) {
            $numbers[$i] = $low_val;
            $numbers[$n - $i - 1] = $high_val;
        }
    ...
    public static function describe(array $numbers): array
    {
        return [
            ...
            'winsorized_mean'     => self::winsorizedMean($numbers, 20),
        ];
    }
}
